...
- **The fixtures are real terraform runs, and `apply-killed` was really killed.**
  ...
  built-in `terraform_data`. They were produced with **OpenTofu 1.10.6**, which is
  what the authoring session could reach; regenerating them with terraform 1.15.5
  on the devbox is the first validation step below.
- Cost: **$0**. Nothing applied, no AWS API called.
- Validation:
```bash
...
- Next allowed step: **20b.2** — one cycle. Publish a timeline from a real apply
  and a real destroy, confirm the schema against terraform 1.15.5 rather than
  OpenTofu, and light the map's nodes from it. About $0.03.
